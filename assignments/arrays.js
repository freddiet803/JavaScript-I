...
oldCars.sort();
let lengthOfOldCars = oldCars.length;
console.log('There are ' + lengthOfOldCars + ' old cars.');

// ==== Challenge 6 ====
// A buyer is interested in seeing only BMW and Audi cars within the inventory.  Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
let BMWAndAudi = [];
for (let q = 0; q < inventory.length; q++) {
  if (inventory[q].car_make == 'BMW' || inventory[q].car_make == 'Audi') {
    BMWAndAudi.push(inventory[q]);
  }
}
BMWAndAudi.sort();
let lastBmwAudi = JSON.stringify(BMWAndAudi);
//console.log(BMWAndAudi.length);
console.log(lastBmwAudi);
